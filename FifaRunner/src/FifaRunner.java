...
public class FifaRunner {
	...
	private static Map<Integer, Integer> ageGroups = new TreeMap<Integer, Integer>();
	private static Map<String, TreeSet<String>> allPositions = new TreeMap<String, TreeSet<String>>();
	private static Map<String, TreeSet<Integer>> mostPositions = new TreeMap<String, TreeSet<Integer>>();
	...
	private static void processLine(int age, String position, String nationality) {
		...
		int group = 5 * (int)Math.ceil(age / 5.0);
		ageGroups.put(group, ageGroups.getOrDefault(group, 0) + 1);

		TreeSet<String> positions = allPositions.getOrDefault(nationality, new TreeSet<String>());
		positions.add(position);
		allPositions.put(nationality, positions);
	...
	private static void printResults() {
		...
		String country = "";
		int highest = 0;
		for (String k : allPositions.keySet()) {

			System.out.println(k + ": " + allPositions.get(k));
			TreeSet<String> current = allPositions.get(k);
			if (current.size() > highest) {
				
				highest = current.size();
				country = k;
		...
		System.out.println("Country with most positions: " + country + "(_" + highest + "_)");
	}
